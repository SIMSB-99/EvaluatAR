...
public class FacialDetection : MonoBehaviour
{
    ...
    [Serializable]
    public struct FaceDetectionRecord
    {
    ...
    private readonly List<FaceDetectionRecord> _lastFaceDetections = new List<FaceDetectionRecord>();

    /// <summary>
    /// Returns the most recent face detections formatted in the project's "pipe JSON" style
    /// (no commas, safe to append as a single CSV column).
    /// </summary>
    public string GetLastDetectionsPipeString()
    {
        var sb = new StringBuilder();
        sb.Append("{\"type\":\"faces\" | \"count\":\"").Append(_lastFaceDetections.Count).Append("\"");
        sb.Append(" | \"faces\":\"");

        for (int i = 0; i < _lastFaceDetections.Count; i++)
        {
            var f = _lastFaceDetections[i];
            sb.Append("{\"id\":\"").Append(i).Append("\" | \"conf\":\"").Append(f.confidence.ToString("F3")).Append("\"");
            sb.Append(" | \"bbox\":\"(")
              .Append(f.x1.ToString("F1")).Append(" | ")
              .Append(f.y1.ToString("F1")).Append(" | ")
              .Append(f.x2.ToString("F1")).Append(" | ")
              .Append(f.y2.ToString("F1")).Append(")\"");
            sb.Append(" | \"center\":\"(")
              .Append(f.cx.ToString("F1")).Append(" | ")
              .Append(f.cy.ToString("F1")).Append(")\"}");
            if (i < _lastFaceDetections.Count - 1) sb.Append(" ~ ");
        }
    ...
    public float confThreshold = 0.8f;
    ...
    public Mat MatFromTexture(Texture2D rawVideoTexturesRGBA)
    {
        ...
        // public Mat RunDetection(Texture2D rawVideoTexturesRGBA)
        // public Tuple<Mat, List<Vector2>> RunDetection(Texture2D rawVideoTexturesRGBA)
    public Tuple<Mat, List<Vector2>, List<drawPredInfo>> RunDetection(Texture2D rawVideoTexturesRGBA)
    {
        rgbaMat = new Mat(size, CvType.CV_8UC3);
        bgrMat = new Mat(size, CvType.CV_8UC3);
        ...
        if (net == null)
        {
            ...
            // return new Tuple<Mat, List<Vector2>>(matrix, listOfVector2);
            List<drawPredInfo> drawPredInfoList = new List<drawPredInfo>();
            return new Tuple<Mat, List<Vector2>, List<drawPredInfo>>(matrix, listOfVector2, drawPredInfoList);
        }
        else
        {
    ...
    // protected Mat postprocess(Mat frame, List<Mat> outs, Net net, int backend = Dnn.DNN_BACKEND_OPENCV)
    // protected Tuple<Mat, List<Vector2>> postprocess(Mat frame, List<Mat> outs, Net net, int backend = Dnn.DNN_BACKEND_OPENCV)
    protected Tuple<Mat, List<Vector2>, List<drawPredInfo>> postprocess(Mat frame, List<Mat> outs, Net net, int backend = Dnn.DNN_BACKEND_OPENCV)
    {
        ...
        List<Vector2> outputFacesCenters = new List<Vector2>();
        List<drawPredInfo> drawPredInfoForFaceCubeList = new List<drawPredInfo>();
        _lastFaceDetections.Clear();
        ...
        for (int i = 0; i < indices.total(); ++i)
        {
            ...
            drawPredInfo drawPredInfoForFaceCube = new drawPredInfo(0, confidence_arr[0], bbox_arr[0], bbox_arr[1], bbox_arr[0] + bbox_arr[2], bbox_arr[1] + bbox_arr[3]);
            drawPredInfoForFaceCubeList.Add(drawPredInfoForFaceCube);
            ...
            */
            ...
            //Account for flipped Y axis
            Vector2 testLoc = new Vector2((bbox_arr[0] + bbox_arr[2]) - (bbox_arr[2] / 2), (float)rows - (bbox_arr[1] + (bbox_arr[3] / 2)));
            outputFacesCenters.Add(testLoc);
            // Debug.Log("Center of Face: " + testLoc);

            // Store detection in a CSV-safe (comma-free) representation.
            _lastFaceDetections.Add(new FaceDetectionRecord
            {
                confidence = confidence_arr[0],
                x1 = bbox_arr[0],
                y1 = bbox_arr[1],
                x2 = bbox_arr[0] + bbox_arr[2],
                y2 = bbox_arr[1] + bbox_arr[3],
                cx = testLoc.x,
                cy = testLoc.y
            });
        ...
        // return frame;
        return new Tuple<Mat, List<Vector2>, List<drawPredInfo>>(frame, outputFacesCenters, drawPredInfoForFaceCubeList);
    }
    ...
    public Mat drawPred(int classId, float conf, double left, double top, double right, double bottom, Mat frame)
    {
        // Imgproc.rectangle(Mat img, Point pt1, Point pt2, Scalar color, int thickness [-1 = Filled])	
        Imgproc.rectangle(frame, new Point(left, top), new Point(right, bottom), new Scalar(0, 0, 0, 255), -1);
