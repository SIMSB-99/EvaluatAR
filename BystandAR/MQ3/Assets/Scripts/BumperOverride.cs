...
public class BumperOverride : MonoBehaviour
{
    ...
    [Header("EvaluatAR Integration")]
    [SerializeField]
    private GameObject EvaluatARObject;
    ...
    void Start()
    {
        ...
        if (EvaluatARObject != null)
        {
            EvaluatARScript = EvaluatARObject.GetComponent<EvaluatAR>();
        }
    ...
    void Update()
    {
        ...
        CameraFrame = CameraCaptureScript.cameraFrame;
        if (CameraFrame != null)
        {
            CameraFrameTexture2D.SetPixels(CameraFrame.GetPixels());
            CameraFrameTexture2D.Apply();

            // Keep a reference for optional image saving + EvaluatAR reference capture
            tempTexture = CameraFrameTexture2D;
            if (EvaluatARScript != null)
            {
                EvaluatARScript.setCurrentCameraCapture(CameraFrameTexture2D);
            }
        ...
        toggleButtonStatus = (EvaluatARScript != null) ? EvaluatARScript.getToggleButtonState() : CameraCaptureScript.toggleButtonState;


        if (FrameCounter % LoggingFrameStep == 0)
        {
            // CSV Logger
            LogFrameToEvaluatAR(faceCubesInScene);
        }
...
private void LogFrameToEvaluatAR(GameObject[] faceCubes)
{
    if (EvaluatARScript == null) return;

    int fps = (int)(1.0f / Time.smoothDeltaTime);

    // Matches old FPSCalculator behavior exactly:
    // qrDetectionStatus was computed as: qrDetectionStatus = !initiallyPositioned
    bool qrDetectionStatus = (CameraCaptureScript != null) ? !CameraCaptureScript.initiallyPositioned : true;

    bool hardcodedEyeGaze = EvaluatARScript.getToggleButtonState();

    string detectedFacesInfo = BuildDetectedFacesInfoPipeJson(faceCubes);

    // IMPORTANT: logData must be CSV-safe (no commas). Use pipes instead.
    string logData =
        "{\"FPS\":\"" + fps + "\" | " +
        "\"# Faces\":\"" + (faceCubes != null ? faceCubes.Length : 0) + "\" | " +
        "\"frame #\":\"" + FrameCounter + "\" | " +
        "\"QRDetection Status\":\"" + qrDetectionStatus + "\" | " +
        "\"Hardcoded Eye Gaze\":\"" + hardcodedEyeGaze + "\" | " +
        "\"DetectedFaces Info\":" + detectedFacesInfo +
        "}";

    EvaluatARScript.writeToLogsFile(logData);
}

private string BuildDetectedFacesInfoPipeJson(GameObject[] faceCubesInScene)
{
    string finalJSONString = "{";

    if (faceCubesInScene != null)
    {
        for (int i = 0; i < faceCubesInScene.Length; i++)
        {
            var faceCubeScript = faceCubesInScene[i].GetComponent<FaceCubeScript>();
            ...
            // Use pipes instead of commas so the entire string stays CSV-safe.
