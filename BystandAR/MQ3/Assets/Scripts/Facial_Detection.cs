...
public class FacialDetection : MonoBehaviour
{
    ...
    public static int captureHeight = 960;
    ...
    protected virtual void Start()
    {
        ...
        if (!string.IsNullOrEmpty(classes))
        {
            ...
            if (string.IsNullOrEmpty(classes_filepath)) Debug.Log("BystandAR -- The file:" + classes + " did not exist in the folder �Assets/StreamingAssets/OpenCVForUnity/dnn�.");
        }
        if (!string.IsNullOrEmpty(config))
        {
            config_filepath = Utils.getFilePath("OpenCVForUnity/dnn/" + config);
            if (string.IsNullOrEmpty(config_filepath)) Debug.Log("BystandAR -- The file:" + config + " did not exist in the folder �Assets/StreamingAssets/OpenCVForUnity/dnn�.");
        }
        if (!string.IsNullOrEmpty(model))
        {
            model_filepath = Utils.getFilePath("OpenCVForUnity/dnn/" + model);
            if (string.IsNullOrEmpty(model_filepath)) Debug.Log("BystandAR -- The file:" + model + " did not exist in the folder �Assets/StreamingAssets/OpenCVForUnity/dnn�.");
        }
        Run();
    }
    ...
    protected virtual void Run()
    {
        ...
        {
            Debug.LogError("BystandAR -- " + model + " is not loaded. Please see �Assets/StreamingAssets/OpenCVForUnity/dnn/setup_dnn_module.pdf�. ");
        }
        else
        {
            //! [Initialize network]
    ...
    public Tuple<Mat, List<Vector2>, List<drawPredInfo>> RunDetection(Texture2D rawVideoTexturesRGBA)
    {
        ...
        {
            Utils.texture2DToMat(rawVideoTexturesRGBA, rgbaMat);
            // Utils.webCamTextureToMat(rawVideoTexturesRGBA, rgbaMat);
        }
