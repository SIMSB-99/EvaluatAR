...
[SerializeField]
    ...
    void Awake()
    {
        ...
        {
            evaluatAR = EvaluatARObject.GetComponent<EvaluatAR>();
        }
    ...
    void Update()
    {
        ...
        if (evaluatAR != null)
        {
            toggleButtonState = evaluatAR.getToggleButtonState();
            elapsedMilliseconds = evaluatAR.getElapsedTimeInMilliseconds();
            evaluatAR.setPositionedStatus(initiallyPositioned);
            evaluatAR.setQrDetectionStatus(!initiallyPositioned);
        }
    ...
    public void PositioningCubePosUpdate()
    {
        ...
        Vector3 positioningCubePos = currQRPosition - (midDir.normalized * (QRDistMid * distScalingFactor));
    ...
    public bool BoundsCheck()
    {
        if (PositioningCubeCollider.bounds.Contains(MainCameraPosition))
        {
            if (!initiallyPositioned)
            {
                ...
                if (evaluatAR != null)
                {
                    evaluatAR.setPositionedStatus(true);
                    evaluatAR.setQrDetectionStatus(false);
                }
            }
