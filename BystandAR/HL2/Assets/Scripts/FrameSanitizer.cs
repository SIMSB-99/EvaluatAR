...
namespace BystandAR
{
    ...
    public class FrameSanitizer : MonoBehaviour
    {
        ...
        [SerializeField] private ArUcoMarker arUcoMarker;
        [SerializeField] private int LoggingFrameStep = 1;

        private int _logFrameCounter = 0;
        private int _latestFaceDetectionsCount = 0;
    ...
    HL2ResearchMode researchMode;
        ...
        async void Start()
        {
            ...
            if (evaluatAR == null)
            {
                evaluatAR = FindObjectOfType<EvaluatAR>();
            }
            if (arUcoMarker == null)
            {
                arUcoMarker = FindObjectOfType<ArUcoMarker>();
            }
        ...
        async void Update()
        {
            ...
            bool didOffload = false;
            int offloadImageBytes = 0;
            int offloadDepthBytes = 0;
        ...
        if (_mediaCaptureUtility.IsCapturing)
        {
                    ...
                    if (SanitizeFrames)
                    {
                            ...
                            && clientSocketImagesScript.connectedToServer && clientSocketDepthScript.connectedToServer))
                        {
                            frameCaptureCounter = 0;
                                                        ...
                                                        var jpgBytes = sanitizedFrame.sanitizedImageFrame.EncodeToJPG();
                            clientSocketImagesScript.inputFrames.Enqueue(jpgBytes);
                            didOffload = true;
                            offloadImageBytes = (jpgBytes != null) ? jpgBytes.Length : 0;
                            if (sanitizedFrame.sanitizedDepthFrame != null)
                            {
                                clientSocketDepthScript.inputFrames.Enqueue(sanitizedFrame.sanitizedDepthFrame);
                                offloadDepthBytes = (sanitizedFrame.sanitizedDepthFrame != null) ? sanitizedFrame.sanitizedDepthFrame.Length : 0;
                            }
            ...
            {
                _logFrameCounter++;
                if (LoggingFrameStep <= 1 || (_logFrameCounter % LoggingFrameStep == 0))
                {
                    LogEvaluatARFrame(didOffload, offloadImageBytes, offloadDepthBytes);
                }
            }
        }
    ...
    {
        _latestFaceDetectionsCount = result.Faces.Count();
    ...
    private byte[] RetreiveDepthFrame()
    {
        ...
        private SanitizedFrames SanitizeFrame(Frame returnFrame, byte[] depthFrame)
        {
            ...
            if (evaluatAR != null)
            {
                evaluatAR.setCurrentCameraCapture(tempImageTexture);
            }
        ...
        private static string Vec3ToPipe(Vector3 v)
        {
            return "<" + v.x + " | " + v.y + " | " + v.z + ">";
        }

        private void LogEvaluatARFrame(bool didOffload, int offloadImageBytes, int offloadDepthBytes)
        {
            ...
            try
            {
                bboxCount = GameObject.FindGameObjectsWithTag("BoundingBox").Length;
                bboxUserCount = GameObject.FindGameObjectsWithTag("BoundingBoxHL2User").Length;
            }
            catch { }

            float qrDist = -1f;
            Vector3 qrDir = Vector3.zero;
            if (arUcoMarker != null)
            {
                qrDist = arUcoMarker.getLatestDistance();
                qrDir = arUcoMarker.getLatestDirection();
            }

            bool imgConnected = (clientSocketImagesScript != null) && clientSocketImagesScript.connectedToServer;
            bool depthConnected = (clientSocketDepthScript != null) && clientSocketDepthScript.connectedToServer;

            string ts = DateTime.Now.ToString("HH-mm-ss.ffff");
            // Pipe-JSON style payload (no commas so it stays in EvaluatAR\'s final CSV column cleanly)
            string payload =
                "{\"type\":\"FrameSanitizer\"}"
                + "|{\"LegacyTimestamp\":\"" + ts + "\"}"
                + "|{\"FPS\":\"" + fps + "\"}"
                + "|{\"FaceDetections\":\"" + _latestFaceDetectionsCount + "\"}"
                + "|{\"FaceCubes\":\"" + bboxCount + "\"}"
                + "|{\"UserCubes\":\"" + bboxUserCount + "\"}"
                + "|{\"SamplingInterval\":\"" + samplingInterval + "\"}"
                + "|{\"SanitizeFrames\":\"" + SanitizeFrames + "\"}"
                + "|{\"OffloadEnabled\":\"" + OffLoadSanitizedFramesToServer + "\"}"
                + "|{\"OffloadedThisFrame\":\"" + didOffload + "\"}"
                + "|{\"OffloadImageBytes\":\"" + offloadImageBytes + "\"}"
                + "|{\"OffloadDepthBytes\":\"" + offloadDepthBytes + "\"}"
                + "|{\"ImgClientConnected\":\"" + imgConnected + "\"}"
                + "|{\"DepthClientConnected\":\"" + depthConnected + "\"}"
                + "|{\"RecordEyeGazePosition\":\"" + recordEyeGazePosition + "\"}"
                + "|{\"UserSpeaking\":\"" + userSpeaking + "\"}"
                + "|{\"MicAmplitude\":\"" + averageAmplitude + "\"}"
                + "|{\"QRDistance\":\"" + qrDist + "\"}"
                + "|{\"QRDirection\":\"" + Vec3ToPipe(qrDir) + "\"}";

            if (evaluatAR != null)
            {
                evaluatAR.writeToLogsFile(payload);
            }
            else
            {
                Debug.Log(payload);
